...
def get_indicator_line(indicatorKey: str, indicatorName: str, client: ChatGPTClient) -> str:
    topics = [client.labels[topic]['short'] for topic in client.get_topics_for_indicator(indicatorKey)]
    topics_line = f"In den Bereichen: {', '.join(topics)}" if len(topics) != 1 else f"Im Bereich: {topics[0]}"
    return f"\"{indicatorName}\": \"{client.labels[indicatorKey]['text']}. {topics_line}\""
